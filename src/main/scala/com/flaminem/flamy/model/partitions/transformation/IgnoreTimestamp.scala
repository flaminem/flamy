...
/**
  ...
  */
class IgnoreTimestamp(
  override val text: String,
  override val table: TableName
) extends Annotation {

  override def check(parents: Seq[Table]): Unit = {
    parents.find{_.fullName == table.fullName} match {
      case None =>
        throw new FlamyParsingException(s"Table ${table.fullName} is not a parent table of this table. " +
          s"Known parent tables are ${parents.map{_.tableName}.mkString("[", ", ", "]")}")
      case _ => ()
    }
  }
